 ...
 */
public class ExVerticalScaleScrollView extends BaseScaleView {

    public ExVerticalScaleScrollView(Context context) {
        super(context);
    }

    public ExVerticalScaleScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public ExVerticalScaleScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ExVerticalScaleScrollView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
    }

    @Override
    protected void initVar() {
        mScaleMargin = (int)TypedValue.applyDimension(COMPLEX_UNIT_MM,1, getResources().getDisplayMetrics());
...
//        canvas.drawLine(mRectWidth, 0, mRectWidth, mRectHeight, paint);
    ...
    protected void onDrawScale(Canvas canvas, Paint paint) {
        ...
        float top = TypedValue.applyDimension(COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics());
        for (int i = 0, k = mMin; i <= mMax - mMin; i++) {
            if (i % 5 == 0) { //整值
                canvas.drawLine(mRectWidth / 2, i * mScaleMargin + top, mRectWidth, i * mScaleMargin + top, paint);
                if (i % 10 == 0) {
                    //整值文字
                    canvas.drawText(String.valueOf(k), 40, i * mScaleMargin + paint.getTextSize() / 3 + top, paint);
                    k += 10;
                }
            } else {
                canvas.drawLine(mRectWidth / 3 * 2, i * mScaleMargin + top, mRectWidth, i * mScaleMargin + top, paint);
            }
        }
    }
    ...
    protected void onDrawPointer(Canvas canvas, Paint paint) {
       ...
       /* paint.setColor(Color.RED);
        ...
        canvas.drawLine(mRectWidth, countScale * mScaleMargin + finalY,
