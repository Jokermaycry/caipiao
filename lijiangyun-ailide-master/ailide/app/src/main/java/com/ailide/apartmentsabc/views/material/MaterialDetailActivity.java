...
import com.ailide.apartmentsabc.model.MaterialDetails;
...
public class MaterialDetailActivity extends BaseActivity {
    ...
    private CollectMaterialGroup collectMaterialGroup;
    ...
    private void initView() {
        ...
        if (getIntent().hasExtra("collects")) {
            collectMaterialGroup = (CollectMaterialGroup) getIntent().getSerializableExtra("collects");
            collectMaterials.addAll(collectMaterialGroup.getData());
        }
    ...
    private void getData() {
        OkGo.<String>post(Urls.MATERIA_TAG_MORE)
                .tag(this)
                .params("tag_id", getIntent().getStringExtra("id"))
                .execute(new StringCallback() {
                    @Override
                    public void onSuccess(Response<String> response) {
                        if (response.isSuccessful()) {
                            MaterialDetails details = JSON.parseObject(response.body(), MaterialDetails.class);
                            materials.clear();
                            materials.addAll(details.getData().getMaterial());
                            mAdapter.notifyDataSetChanged();
                            NetWorkImageLoader.loadCircularImage(null, Urls.BASE_IMG + details.getData().getTag().getImage_url(), mIvAvatar, R.drawable.icon_circle_mrtx, R.drawable.icon_circle_mrtx);
                            mTvTitle.setText(details.getData().getTag().getTag_name());
                            mTvGroup.setText(getIntent().getStringExtra("class"));
                            mTvCollectNum.setText(details.getData().getTag().getCollection_r() + "");
                            mTvSaveNum.setText(details.getData().getTag().getPrint_num_r() + "");
                            materialGalleryAdapter.notifyDataSetChanged();
                        }
                    }
                });
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.iv_print:
                if (PrintFragment.printPP != null && PrintFragment.printPP.isConnected()) {
                    ...
                    intent.putExtra("materialId", materials.get(mVpGallery.getCurrentItem()).getId());
    ...
    private void showPopView() {
        ...
        pop_equitment_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                popupWindow.dismiss();
                jumpToOtherActivity(new Intent(MaterialDetailActivity.this, MyEquitmentActivity.class), false);
            }
        });
