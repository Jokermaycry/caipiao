...
public class StickerView extends FrameLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                downY = ev.getY();
                if (isSelectLast) {
                    if (!((handlingSticker != null && isInStickerArea(handlingSticker, downX, downY)) || findCurrentIconTouched() != null)) {
                        isSelectLast = false;
                        isSelect = false;
                        if (onStickerOperationListener != null) {
                            onStickerOperationListener.onStickerCancel();
                        }
                        invalidate();
                        return super.onInterceptTouchEvent(ev);
                    }
                }
                boolean onSticker = findCurrentIconTouched() != null || findHandlingSticker() != null;
                if (!onSticker) {
                    isSelectLast = onSticker;
                }
                invalidate();
                return onSticker;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                if (currentMode == ActionMode.ICON && handlingSticker != null && handlingSticker instanceof TextSticker) {
                    TextSticker textSticker = (TextSticker) handlingSticker;
                    textSticker.scaleFinish();
                }
                onTouchUp(event);
                invalidate();
                break;
    ...
    protected void onTouchMove(@NonNull MotionEvent event) {
    ...
    protected void onTouchUp(@NonNull MotionEvent event) {
        ...
        if (currentMode == ActionMode.DRAG
            ...
            if (currentTime - downTime < minClickDelayTime) {
                if(isSelectLast && lastHandlingSticker != null && lastHandlingSticker == handlingSticker){
                    isSelect = false;
                    isSelectLast = false;
                    lastHandlingSticker = null;
                    handlingSticker = null;
                }else {
                    isSelect = true;
                    isSelectLast = true;
        ...
        switch (currentMode) {
            ...
            case ActionMode.ICON:
                if (handlingSticker != null && currentIcon != null) {
                    ...
                    onStickerOperationListener.update();
                }
                break;
        }
    }
  ...
  /*  for (int i = 0; i < stickers.size(); i++) {
    ...
    public StickerView addSticker(@NonNull Sticker sticker) {
        return addSticker(sticker, Sticker.Position.BOTTOM);
    }
    ...
    public StickerView addStickerWithHeight(@NonNull final Sticker sticker,
                                            final int height) {
        if (ViewCompat.isLaidOut(this)) {
            addStickerImmediatelyWithHeight(sticker, height);
        } else {
            post(new Runnable() {
                @Override
                public void run() {
    ...
    protected void addStickerImmediatelyWithHeight(@NonNull Sticker sticker, int height) {
        setStickerPositionWithHeight(sticker, height);
        handlingSticker = sticker;
        stickers.add(sticker);
        if (onStickerOperationListener != null) {
            onStickerOperationListener.onStickerAdded(sticker);
     ...
     * @return
     */
    public float getLastLocationY(int locationY) {
        for (Sticker sticker :
                stickers) {
            if (locationY < sticker.getLocationY()) {
...
//      offsetY *= 3f / 4f;
    ...
    protected void setStickerPositionWithHeight(@NonNull Sticker sticker, int height) {
        float width = getWidth();
        float[] values = new float[9];
        sticker.getMatrix().getValues(values);
        float offsetX = width - sticker.getWidth() * values[0];
        float offsetY = height - sticker.getHeight() * values[0];
        offsetY /= 2f;
        offsetX /= 2f;
        sticker.getMatrix().postTranslate(offsetX, offsetY);
    }
